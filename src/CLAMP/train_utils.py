...
def train_epoch(
    ...
    for d in tqdm(data_loader):
        ...
        CLAMP_labels = d["CLAMP_labels"].to(device)
        CLAMP_labels_2 = d["CLAMP_labels_2"].to(device)
        # print(CLAMP_labels.shape)
        ner_outputs = model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            CLAMP_tags=CLAMP_labels,
            CLAMP_tags_2=CLAMP_labels_2
        )
    ...
    f1_score = classification_report(label_list, pred_list, mode='strict', output_dict=True)
    print(f1_score["micro avg"])
